...
The accompanying Python script, serial_to_txt.py, is designed to process data obtained from the potentiostat during CV measurements. The script utilizes the pyserial module to establish serial communication with the potentiostat over a specified serial port. The main function, parse_data, reads data from a file and applies conversion functions to extract meaningful electric potential and current values (x and y values) for plotting. It processes the raw data, applying averaging to smoothen the results. The processed x and y values are saved to a text file called "new_data.txt," providing organized and pre-processed data for subsequent analysis. Additionally, the script uses the matplotlib library to generate a line plot, visualizing the relationship between electric potential and current with the scan rate as an experimental parameter. The generated plot is saved as "graph.png," enabling researchers to gain valuable insights and perform further data analysis in their research study.
